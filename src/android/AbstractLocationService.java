...
protected void handleLocation (Location location) {
  ...
  try {
    //if(config.getDesiredAccuracy()*20 > bgLocation.getAccuracy()) {
      Log.d(TAG, "Sending updated location to : " + config.getURL()+" "+bgLocation.toJSONObject().toString());
      new HTTPPostTask().execute(bgLocation.toJSONObject().toString());
    //}
    //else {
    //  Log.i(TAG, "Not accurate: "+bgLocation.getAccuracy());
    //}
  }
  catch(Exception e) {
...
public void onTaskRemoved(Intent rootIntent){
  ...
  restartServiceIntent.putExtra("config", config.toParcel().marshall());
...
private class HTTPPostTask extends AsyncTask<String, Void, Void> {
  @Override
  protected Void doInBackground(String... dataArr) {
    try  {
      String data = dataArr[0];
      Log.i(TAG, "Location: "+data);
      String dataStr = "location="+data;
      ...
      Log.w(TAG, "Failed to send position");
